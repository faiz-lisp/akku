...
(let-values (((major minor sub-minor) (scheme-version-number)))
  (let ((version (format #f "~d.~d.~d" major minor sub-minor))
    ...
    (let ((default-heap-path (format #f "lib/csv~d/~d" version machine))
          (petite (which "petite")))
      (unless petite
      ...
      (let* ((heap-dir (path-join (path-parent (path-parent petite)) default-heap-path))
        ...
        ;; Build a tarball.
        (let* ((build-version (format #f "~d+pcsv~d.~d" akku-version version machine))
